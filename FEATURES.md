...
Last updated 15 Aug 2026. Design detail lives in [design/](design/), remaining work in
...
| CI/CD with gated migrations + isolation as a separate gate | planned |
...
| Guard app offline mode with outbox sync | in-progress | server sync endpoint done; handset starting |
| SOS panic button — medical, fire, gas, security | planned | table exists, no endpoints |
...
| Amenity booking with DB-level double-booking prevention | in-progress | exclusion constraint done and tested; no endpoints |
| Document repository with role-based access | planned |
| Parking slot allocation | completed | plus vehicles and violation flagging |
| Society admin console | in-progress | 5 pages; no screens yet for the Phase 2 modules |
